...
def search():
    totalPc = InitParams.totalOwnPiece - 1
    kVal = InitParams.kValue
    curThr = getNumberOfThreat()
    #Board.totalThreat = getNumberOfThreat()
    while(totalPc > kVal):
    ...
    for pieces in Board.dictOfPieces:
        ...
        if (curThreat <= numOfThreat):
            if (curThreat != numOfThreat):
                listOfPos = []
                count = 0
            listOfPos.append(curPc)
            pos = curPc
            count+=1
            numOfThreat = curThreat #uncomment to get steepest descend
    randIdx = randrange(0,count)
    #print("count: ", count)
    #print("list: ", listOfPos)
    pos = listOfPos[randIdx]
    return pos
