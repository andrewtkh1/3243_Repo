...
class Board:
    ...
    totalCount = 0
    visited = {} #Dict
    inPqDict = {} #Contains Destination Node & Cost so far.
    currentPathCost = 0 #Total path cost to the current node.
    poppedDict = {}
    markAsDeleted = {}
...
class PQ:
    ...
    def pop(self):
        if (self.count == 0):
            raise KeyError("Popped empty PQ")
        self.count-=1
        fn, idx, node = heapq.heappop(self.pqList)
        return node
    ...
    def printPQ(self):
        print("Current Items in PQ:")
        for i in self.pqList:
            print(i, i[2].totalCost, end=" ")
        print("")
...
def run_AStar():
    ...
    moves, nodesExplored, totalCost = search() #For reference
    #print(moves)
    #print(nodesExplored)
    return moves, nodesExplored, totalCost #Format to be returned
...
def search():
    ...
    for i in startList:
        ...
        frontier.push(i) #Add all actions from 1st node
    ...
    while (not frontier.isEmpty()):
        ...
        #print("Pop :", nextPos, " Cost: ", curNode.totalCost)
